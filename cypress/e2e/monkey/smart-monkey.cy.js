//tomado de https://github.com/TheSoftwareDesignLab/monkey-cypress 
...
let LOG_FILENAME = "../../../results/monkey-execution.html";
        ...
        if($inputs.length > 0){
            ...
            if(!Cypress.dom.isHidden(inp)) {
                ...
                if(inp.getAttribute("type") == "email"){
                    ...
                    let type = faker.random.alphaNumeric+'a'
                    cy.wrap(inp).type(type,{ parseSpecialCharSequences: false, force: true });
                    info = `Input ${inp.id} was filled with ${type}`
                }
                else {
                    focused = false;
...
describe( `${appName} under smarter monkeys`, function() {
    ...
    it(`smart monkey test de la aplicación ${appName} para la funcionalidad de páginas`, function() {
        ...
        if(pcg === 100){
            ...
            url = 'http://localhost:2368/ghost/#/editor/page';
            cy.visit(url).then((win)=>{   

                cy.get('input[name="identification"]').type('teste2e@test.com')
                cy.get('input[name="password"]').type('Teste2e12345*')
    ...
    it(`smart monkey test de la aplicación ${appName} para la funcionalidad de Tags`, function() {
        if(!seed) seed = getRandomInt(0, Number.MAX_SAFE_INTEGER);
        ...
        if(pcg === 100){
            ...
            pending_events[6] = events*pct_browserChaos/100;
            pending_events[7] = events*pct_actions/100;
            url = 'http://localhost:2368/ghost/#/tags/new';
            cy.visit(url).then((win)=>{   
